...
import com.atguigu.gmall.realtime.dwd.test.bean.Dept;
import com.atguigu.gmall.realtime.dwd.test.bean.Emp;
...
public class Test02_SQL_JOIN {
    public static void main(String[] args) {
        ...
        // 在生产环境中，一定设置状态的保留时间。  tableEnv.getConfig().setIdleStateRetention(Duration.ofSeconds(10));
        // tableEnv.executeSql("select e.empon,e.ename,d.deptno,d.dname from emp e join dept d on e.deptno = d.deptno").print();

        // TODO 6.左外连接
        // 注意：如果是左表数据先到，右表数据后到，会产生3条数据
        // 左表   null    标记为+I
        // 左表   null    标记为-D
        // 左表   右表    标记为+I
        // 这样的动态表转换的流称之为回撤流
        // tableEnv.executeSql("select e.empon,e.ename,d.deptno,d.dname from emp e left join dept d on e.deptno = d.deptno").print();
        // TODO 7.右外连接
        // tableEnv.executeSql("select e.empon,e.ename,d.deptno,d.dname from emp e right join dept d on e.deptno = d.deptno").print();
        // TODO 8.全外连接
        // tableEnv.executeSql("select e.empon,e.ename,d.deptno,d.dname from emp e full join dept d on e.deptno = d.deptno").print();


        // TODO 9.将左外连接的结果写到kafka主题中
        // 注意：kafka连接器不支持写入的时候包含update或者delete操作，需要用upsert-kafka连接器
        // 如果左外连接，如果左表数据先到，右表数据后到。会有3条结果产生
        // 左表   null    标记为+I
        // 左表   null    标记为-D
        // 左表   右表    标记为+I
        ...
        // null
        // 左表   右表
        // 当从kafka主题中读取数据的时候，存在空消息，如果使用的FlinkSQL的方式读取，会自动的将null消息过滤掉；如果使用的是FlinkAPI的方式读取的话，默认的SimpleStringSchema是处理不了空消息的，需要自定义反序列化器
        ...
        // 9.2 写入
        tableEnv.executeSql("insert into emp_dept select e.empon,e.ename,d.deptno,d.dname from emp e left join dept d on e.deptno = d.deptno");
